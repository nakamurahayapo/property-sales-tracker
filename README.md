...
- **値下げ検討アラート**：決済予定日を1ヶ月半（1ヶ月＋15日）過ぎても登録されたまま（＝未成約）の物件に「値下げ検討」タグを表示。あくまで画面上の注意喚起で、価格自体は今まで通り人が編集します（自動での価格書き換えは行いません）。閾値は `js/config.js` の `PRICE_REVIEW_MONTHS_AFTER_SETTLEMENT` / `PRICE_REVIEW_DAYS_AFTER_SETTLEMENT` で変更できます
